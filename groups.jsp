<%@page import="career.connection.dao" %>
<%@page import="java.sql.*" %>
<%@page import="java.io.*" %>

<!Doctype html>

<html>
<head>
	<link rel="stylesheet" type="text/css" href="style.css" />
	<title>MailSender</title>
	<script>
		function cat(x)
		{
			document.getElementById('tab1').style.display='none';
			document.getElementById('tab2').style.display='none';
			document.getElementById('tab'+x).style.display='block';
		}
	</script>
</head>

<body>
<div id="container">

	<%
		String un = (String)session.getAttribute("logn");
	%>

	<div id="header">
        	<h1>Mail<span class="off">Sender.com</span></h1> 
       

		
		<%
	
		File fe = new File(getServletContext().getRealPath("/")+"/photos/");
		String str[] = fe.list();
		for(int i=0 ; i<str.length ; i++)
		{
			String fn = str[i].substring(0,str[i].indexOf("."));
			fn = fn + ".com";
			if(fn.equals(un))
			{
			out.print("<h2>Hi, "+un.substring(0,un.indexOf("@"))+" <img src='photos/"+str[i]+"' width='40' height='40'></img> </h2>");
			break;
			}	
			else
			{
			out.print("<h2>Hi, "+un.substring(0,un.indexOf("@"))+" <img src='photos/default.png' width='40' height='40'></img> </h2>");
			}
		}
		%>

	 </div>   
      
        <div id="menu">
        	<ul>
            	<li class="menuitem"><a href="profile.jsp">Home</a></li>
                <li class="menuitem"><a href="settings.jsp">Settings</a></li>
                <li class="menuitem"><a href="editprofile.jsp">Profile</a></li>
                <li class="menuitem"><a href="logout.jsp">Logout</a></li>
                </ul>
        </div>
        
        <div id="leftmenu">

        	<div id="leftmenu_top"></div>
		<div id="leftmenu_main">    
	        	<h3>Links</h3>
             		<ul>
                	    <li><a href="compose.jsp">Compose</a></li>
                	    <li><a href="inbox.jsp">Inbox</a></li>
                	    <li><a href="draft.jsp">Draft</a></li>
                	    <li><a href="sent.jsp">Sent Items</a></li>
                	    <li><a href="important.jsp">Important</a></li>
                	    <li><a href="trash.jsp">Trash</a></li>
                	    <li><a href="groups.jsp">Groups</a></li>
                	    <li><a href="contact.jsp">Contact</a></li>
                	</ul>
		</div>
        	<div id="leftmenu_bottom"></div>
        </div>
        
	<div id="content">
        	<div id="content_top"></div>
        	<div id="content_main">
			<h3>Groups</h3>
			<br>
			<a href='#' onclick="cat('1')">Create New Group</a> | <a href="#" onclick="cat('2')">View Existing Group</a>
			<br><br>
			<table id="tab1" style="display:block">
			<form action="creategroup.jsp">
			<tr><td>Group Name <input type="text" name="groupname"></input> <input type="submit" value="create"></input></td></tr>
			<tr><td><br><b>Contact List</b></td></tr>
			<tr><td>
			<%
				Connection con = dao.createconnection();
				PreparedStatement ps = con.prepareStatement("select * from mailsender_contact where login_id=?");
				ps.setString(1,un);
				ResultSet rs = ps.executeQuery();
				while(rs.next())
				{
					out.print("<input type='checkbox' name='addcon' value='"+rs.getString("email")+"'></input>&nbsp;"+rs.getString("email")+"<br>");
				}
				con.close();
			%>
			</td></tr>
			</form>
			</table>

			<table id="tab2" style="display:none" border="1">
			<form action="#">
			<tr><th>Group Name</th><th>Existing Member</th><th>Remove Group</th><th>Edit Group</th><th>Message</th></tr>	
			<%
				Connection con1 = dao.createconnection();
				PreparedStatement ps1 = con1.prepareStatement("select * from MAILSENDER_GROUP where group_logid=?");
				ps1.setString(1,un);
				ResultSet rs1 = ps1.executeQuery();
				while(rs1.next())
				{
					out.print("<tr><td>"+rs1.getString("group_name")+"</td><td><a href='showmembers.jsp?groupname="+rs1.getString("group_name")+"'>show members</a></td><td><a href='removegroup.jsp?groupname="+rs1.getString("group_name")+"'>remove</a></td><td><a href='editgroup.jsp?groupname="+rs1.getString("group_name")+"'>edit</a></td><td><a href='composegroup.jsp?sender="+rs1.getString("group_members")+"'>send</a></td></tr>");
				}
				con1.close();
			%>
			</form>		
			</table>
		</div>
		<div id="content_bottom"></div>
		<div id="footer"></div>
      	</div>

</div>
</body>
</html>

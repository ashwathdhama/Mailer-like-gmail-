...
<div id="container">
	...
	<div id="content">
			...
			<%
				...
				PreparedStatement ps = con.prepareStatement("select * from MAILSENDER_SENTMAILS where sender=? and trash='false'");
				ps.setString(1,un);
				ResultSet rs = ps.executeQuery();
				out.print("<form action='sentmailsdelete.jsp' method='get'>");
				out.print("<table class='tg'>");
				out.print("<th><input type='submit' value='Delete'></input></th><th>Recepient</th><th>Subject</th><th>Date</th>");
				while(rs.next())
				{
					out.print("<tr>");
					out.print("<td><input type='checkbox' name='delmsg' value='"+rs.getString("datetime")+"'></input> <input type='hidden' name='rec' value='"+rs.getString("receiver")+"'/> </td><td>"+rs.getString("receiver")+"</td><td><a href='displaysendmail.jsp?datetime="+rs.getString("datetime")+"&sender="+rs.getString("sender")+"&rec="+rs.getString("receiver")+"'>"+rs.getString("subject")+"</a></td><td>"+rs.getString("datetime")+"</td>");
					out.print("</tr>");
				}	
				out.print("</table>");
				out.print("</form>");
